...
exports.signin = (req,res)=>{
    ...
    .exec((err,user)=>{
        ...
        if(user)
        {
            if(user.authenticate(req.body.password) && user.role=='admin')            // writing "authenticate" calls the function inside "authenticate" method in file "models/user.js" and "req.body.password" is passed as an argument
            {
                const token= jwt.sign({_id: user._id, role: user.role},process.env.JWT_SECRET,{expiresIn: '1h'});    // jwt.sign(payload,secretKey,expiry time) => creates a token
